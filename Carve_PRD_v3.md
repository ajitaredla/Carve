...
| AI Generation | Claude Managed Agents (two agents: `carve-generator`, `carve-verifier`), model `claude-haiku-4-5` | Deviation from the original `claude-sonnet-4-6` choice, made during v1 build (2026-07-25): cheapest current-tier model, an explicit cost-minimization call for v1. $1/M input, $5/M output — cheaper than the figure below implies; §10.2's cost table needs recalculating against this pricing. Built as Managed Agents sessions (not raw API calls) so the generator/verifier can call Carve's own MCP server tools (§10.1) to fetch ground truth instead of trusting prompt-supplied facts. Risk: since the verifier's role (catching drift) is arguably more failure-sensitive than the generator's (a missed verification error is a silent false PASS), an eval pass comparing Haiku's verification accuracy against known-good/known-bad generations is planned before all FR-05 document types ship — see task list task 6.1b. If accuracy is inadequate, only the verifier would need to move to a stronger model, not the generator. |
...
**Note (2026-07-25):** the Claude API row below was sized against `claude-sonnet-4-6` pricing per the original §10 architecture row. The actual v1 model choice is `claude-haiku-4-5` (roughly 3x cheaper on both input and output), so these figures are now a stale upper bound, not a current estimate — needs recalculating before being used for real budget planning. Separately, these figures model a raw two-call API pattern; the actual implementation runs each call as a Managed Agents session (per-session cloud container, session/event-stream lifecycle), which likely carries overhead this table doesn't account for at all. Flagged for whoever owns production cost monitoring — not corrected with real numbers here since no production session-volume data exists yet to size it against.

| Component | Cost at 0–500 Users | Cost at 500–2,000 Users |
|---|---|---|
| Claude API (generation + verification calls, §10.1) — **stale, see note above** | $35–90/month | $250–700/month |
...
| 3.2 | July 25, 2026 | Reconciled §10's AI Generation row with the actual v1 build decision (caught by a product-review gate during implementation, previously undocumented outside a YAML code comment): generation/verification runs as two Claude Managed Agents (`carve-generator`, `carve-verifier`) using `claude-haiku-4-5` — a deliberate cheapest-tier cost choice, not `claude-sonnet-4-6` as §10 originally specified — with the risk noted that the verifier's role is more failure-sensitive than the generator's (a missed verification error is a silent false PASS) and an accuracy eval is planned before all six FR-05 document types ship. §10.2's cost table flagged as stale (sized against Sonnet pricing, and not accounting for Managed Agents' per-session container overhead vs. a raw two-call API pattern) rather than recalculated, since no production session-volume data exists yet to size it against. |
